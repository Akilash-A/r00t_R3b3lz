...
const PixelBlast: React.FC<PixelBlastProps> = ({
  ...
  useEffect(() => {
    ...
    if (mustReinit) {
      ...
      const mapToPixels = (e: MouseEvent | PointerEvent | Touch) => {
        ...
        const clampedFx = Math.max(0, Math.min(renderer.domElement.width, fx));
        const clampedFy = Math.max(0, Math.min(renderer.domElement.height, fy));
      ...
      const handleClick = (fx: number, fy: number) => {
        const ix = threeRef.current?.clickIx ?? 0;
        uniforms.uClickPos.value[ix].set(fx, fy);
        uniforms.uClickTimes.value[ix] = uniforms.uTime.value;
        if (threeRef.current) threeRef.current.clickIx = (ix + 1) % MAX_CLICKS;
        console.log(`Click registered at: ${fx}, ${fy}`); // Debug log
      };
